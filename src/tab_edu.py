...
def render():
    ...
    st.write(
        "**Optical flow** is an example of a **non-linear image registration** method, which can apply an elastic transformation across the image instead of being limited to linear transformations. It estimates a vector field **u(x,y), v(x,y)** with one 2D displacement vector per pixel, containing the number of pixels it moved horizontally (Δx) and vertically (Δy). These vectors form the displacement field or warp field. Typically, when people use optical flow, they only care about retrieving this vector field for motion-tracking purposes, but it can also be used for stabilization (like this app does)."
    )
    st.markdown("""
